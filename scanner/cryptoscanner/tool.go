...
// CheckEncryptionInput is the input type for the check_encryption tool.
...
// RegisterTools registers the check_encryption MCP tool on the given server.
func RegisterTools(server *mcp.Server) {
	s := New()

	mcp.AddTool(server, &mcp.Tool{
		Name:        "check_encryption",
		Description: "Scan Go source files for encryption violations: weak hash algorithms (md5/sha1) with context scoring, hardcoded encryption keys/IVs, and plain HTTP URLs. Maps findings to PCI DSS 6.2.4 and 4.2.1.",
	}, func(ctx context.Context, req *mcp.CallToolRequest, input CheckEncryptionInput) (*mcp.CallToolResult, *scanner.ScannerToolOutput, error) {
		...
		if err != nil {
			return &mcp.CallToolResult{
				Content: []mcp.Content{
					&mcp.TextContent{Text: fmt.Sprintf("check_encryption error: %s", err.Error())},
				},
				IsError: true,
			}, nil, nil
		}
		...
		summary := fmt.Sprintf("check_encryption: %d findings (%d CRITICAL, %d HIGH, %d MEDIUM, %d LOW, %d INFO) in %dms",
			len(out.Findings),
