...
const toolNameCrypto = "check_encryption"

type CheckEncryptionInput struct {
	Path             string   `json:"path" jsonschema:"required,Path to the Go project directory to scan for encryption violations"`
	...
	Cursor           string   `json:"cursor,omitempty" jsonschema:"Opaque cursor token from a prior check_encryption response. When set resumes pagination from the stored session cache (10-minute TTL). Leave empty for a fresh scan."`
...
type cryptoCacher struct{}

func (cryptoCacher) Put(sid string, findings []scanner.Finding, meta hybridcache.ScanMeta) {
	cp := make([]scanner.Finding, len(findings))
	copy(cp, findings)
	hybridcache.Put(sid, &hybridcache.Entry{Findings: cp, Meta: meta})
}

func (cryptoCacher) Get(sid string) ([]scanner.Finding, hybridcache.ScanMeta, bool) {
...
func (cryptoCacher) PutWithHistogram(sid string, findings []scanner.Finding, meta hybridcache.ScanMeta, hist *hybridcache.Histogram) {
	cp := make([]scanner.Finding, len(findings))
	copy(cp, findings)
	hybridcache.PutWithHistogram(sid, cp, meta, hist)
}

func (cryptoCacher) GetWithHistogram(sid string) ([]scanner.Finding, hybridcache.ScanMeta, *hybridcache.Histogram, bool) {
	return hybridcache.GetWithHistogram(sid)
}

func (cryptoCacher) Histogram(findings []scanner.Finding) *hybridcache.Histogram {
...
func RegisterTools(server *mcp.Server) {
	s := New()
	schema, err := buildCryptoOutputSchemaUnion()
	if err != nil {
		slog.Warn("buildCryptoOutputSchemaUnion failed", "err", err)
	}

	tool := &mcp.Tool{
		Name: toolNameCrypto,
		Description: "Scan Go source files for encryption violations: weak hash algorithms (md5/sha1) with context scoring, hardcoded encryption keys/IVs, and plain HTTP URLs. " +
			...
			"Maps findings to PCI DSS 6.2.4, 4.2.1.",
		Meta:         mcp.Meta{"anthropic/maxResultSizeChars": 20000},
		OutputSchema: json.RawMessage(schema),
	}

	mcp.AddTool(server, tool, func(ctx context.Context, req *mcp.CallToolRequest, input CheckEncryptionInput) (*mcp.CallToolResult, any, error) {
		if input.Limit == -1 {
			return cryptoErrorResult("LIMIT_MINUS_ONE_REMOVED: limit=-1 is no longer accepted. For interactive use, call with default params (summary-first with next_cursor) or apply min_severity/rule_filter for a paged flat response; follow the cursor for subsequent pages."), nil, nil
		}
		...
		if input.Cursor != "" && (scopeFilterSet || qualityFilterSet) {
			return cryptoErrorResult("check_encryption cursor_malformed: cursor + filter/scope params is not supported; re-run without cursor to apply new filters"), nil, nil
		}
		...
		buildSummary := func(findings []scanner.Finding, meta hybridcache.ScanMeta, sid, nextCursor string) *CryptoSummaryResponse {
			return buildCryptoSummaryInternal(findings, meta, sid, nextCursor)
		}
		...
		in := hybrid.Input{
			...
			ToolName:      toolNameCrypto,
			FlatPageSize:  30,
		}

		res, sErr := hybrid.SelectAndExecute[scanner.Finding, CryptoSummaryResponse, scanner.ScannerToolOutput](
			ctx, in, scan, filterFunc, buildSummary, buildFlat, cryptoCacher{},
		)
		if sErr != nil {
			return cryptoErrorResult(fmt.Sprintf("check_encryption error: %s", sErr.Error())), nil, nil
		}
		if res.Err != nil {
			cerr := &CryptoCursorError{
				ResponseShape: "error",
		...
		if res.Summary != nil {
			return &mcp.CallToolResult{
				Content: []mcp.Content{&mcp.TextContent{Text: fmt.Sprintf("check_encryption: %d findings (summary with top %d per severity)", res.Summary.Pagination.TotalFindings, TopNPerSeverityCrypto)}},
			}, res.Summary, nil
		}
		return &mcp.CallToolResult{
			Content: []mcp.Content{&mcp.TextContent{Text: fmt.Sprintf("check_encryption: %d findings returned (flat page)", len(res.Flat.Findings))}},
		}, res.Flat, nil
	})
}
...
func cryptoErrorResult(msg string) *mcp.CallToolResult {
	return &mcp.CallToolResult{
