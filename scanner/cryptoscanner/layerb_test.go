...
func newCryptoSessionForLayerB(t *testing.T) *mcp.ClientSession {
	t.Helper()
...
func copyFixtureTreeForCrypto(t *testing.T, srcRoot string) string {
	t.Helper()
...
func callCryptoDefault(t *testing.T, path string) *mcp.CallToolResult {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("CallTool check_encryption: %v", err)
	}
	if result.IsError {
		t.Fatalf("check_encryption IsError: %+v", result)
	}
	return result
}

func cryptoStructuredMap(t *testing.T, result *mcp.CallToolResult) map[string]any {
	t.Helper()
...
func TestCryptoLayerB_Default(t *testing.T) {
	...
	m := cryptoStructuredMap(t, result)
...
func TestCryptoLayerB_TopNPerSeverity_Is3(t *testing.T) {
	...
	if lowArr, _ := top["low"].([]any); len(lowArr) != 0 {
		t.Errorf("top_findings[\"low\"] expected empty (CRYPTO LOW bucket is 0 on fixture); got %d", len(lowArr))
	}
}
...
func TestCryptoLayerB_Deterministic(t *testing.T) {
	...
	r2 := callCryptoDefault(t, scanRoot)
...
func TestCryptoLayerB_SizeBudget20KB(t *testing.T) {
	...
	const budget = 20480
	t.Logf("crypto Layer B wire size on golden fixture = %d bytes (budget %d)", len(raw), budget)
	if len(raw) >= budget {
		t.Fatalf("crypto Layer B wire size %d bytes exceeds budget %d", len(raw), budget)
	}
}
...
func TestCryptoLayerB_CursorRejectsFilterCombo(t *testing.T) {
	...
	result, err := session.CallTool(context.Background(), &mcp.CallToolParams{
		Name: "check_encryption",
		Arguments: map[string]any{
			"cursor":        "eyJzaWQiOiJhYmMiLCJvZmYiOjAsInRvb2wiOiJjaGVja19lbmNyeXB0aW9uIn0",
			"include_tests": true,
		},
	})
	if err != nil {
...
func TestCryptoToolDescription_SummaryFirstBias(t *testing.T) {
	...
	if !found {
		t.Fatalf("check_encryption not in ListTools")
	}
	needles := []string{
		"summary",
		"cursor",
	...
	for _, n := range needles {
		if !strings.Contains(desc, n) {
			t.Errorf("check_encryption description missing substring %q; got: %s", n, desc)
		}
	}
}
...
func TestCryptoLayerA_SizeBudget(t *testing.T) {
	...
	if result.IsError {
		t.Fatalf("check_encryption Layer A returned IsError: %+v", result)
	}
	m := cryptoStructuredMap(t, result)
	if got := m["response_shape"]; got != "flat" {
	...
	const budget = 20480
	t.Logf("crypto Layer A wire size on golden fixture (include_tests=true) = %d bytes (budget %d)", len(raw), budget)
	if len(raw) >= budget {
		t.Fatalf("crypto Layer A wire size %d bytes exceeds budget %d", len(raw), budget)
	}
}
...
func TestCryptoLayerB_MaxResultSizeChars(t *testing.T) {
	...
	for _, tool := range tools.Tools {
		...
		if !ok {
			t.Fatalf("check_encryption Meta missing anthropic/maxResultSizeChars; got %+v", tool.Meta)
		}
		got := int64(0)
		switch x := v.(type) {
		case int:
...
func TestCryptoLayerA_IncludesHistogram(t *testing.T) {
	...
	if result.IsError {
		t.Fatalf("check_encryption Layer A IsError: %+v", result)
	}
	m := cryptoStructuredMap(t, result)
	if got := m["response_shape"]; got != "flat" {
...
func TestCryptoToolDescription_LayerAHistogramNeedle(t *testing.T) {
	...
	for _, n := range needles {
		if !strings.Contains(desc, n) {
			t.Errorf("check_encryption description missing substring %q", n)
		}
	}
}
