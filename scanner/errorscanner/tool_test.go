...
// setupTestServer creates a server with check_error_handling registered,
...
func TestToolRegistered(t *testing.T) {
	...
	for _, tool := range result.Tools {
		if tool.Name == "check_error_handling" {
			found = true
			if tool.Description == "" {
				t.Error("check_error_handling tool should have a non-empty description")
			}
			break
		}
	}
	if !found {
		t.Error("check_error_handling tool should be registered in the server")
	}
}
...
func TestToolValidPath(t *testing.T) {
	...
	if !out.HasRuleID("ERR-LEAK-DIRECT") {
		t.Errorf("Expected ERR-LEAK-DIRECT in findings, got: %+v", out.Findings)
	}
	if !out.HasRuleID("ERR-LEAK-FORMAT") {
		t.Errorf("Expected ERR-LEAK-FORMAT in findings, got: %+v", out.Findings)
	}
	if !out.HasRuleID("ERR-LEAK-ENCODE") {
		t.Errorf("Expected ERR-LEAK-ENCODE in findings, got: %+v", out.Findings)
	}
}
