...
func newErrorSessionForLayerB(t *testing.T) *mcp.ClientSession {
	t.Helper()
	server := mcp.NewServer(&mcp.Implementation{Name: "error-layerb", Version: "v0.0.1"}, nil)
	errorscanner.RegisterTools(server)
	clientTransport, serverTransport := mcp.NewInMemoryTransports()
	go func() {
		_ = server.Run(context.Background(), serverTransport)
	}()
	client := mcp.NewClient(&mcp.Implementation{Name: "error-layerb-test-client", Version: "v0.0.1"}, nil)
...
func copyFixtureTreeForError(t *testing.T, srcRoot string) string {
	t.Helper()
...
func callErrorDefault(t *testing.T, path string) *mcp.CallToolResult {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("CallTool check_error_handling: %v", err)
	}
	if result.IsError {
		t.Fatalf("check_error_handling IsError: %+v", result)
	}
	return result
}

func errorStructuredMap(t *testing.T, result *mcp.CallToolResult) map[string]any {
	t.Helper()
...
func TestErrorLayerB_Deterministic(t *testing.T) {
	...
	r2 := callErrorDefault(t, scanRoot)
...
func TestErrorLayerB_SizeBudget20KB(t *testing.T) {
	...
	const budget = 20480
	t.Logf("error Layer B wire size on golden fixture = %d bytes (budget %d)", len(raw), budget)
	if len(raw) >= budget {
		t.Fatalf("error Layer B wire size %d bytes exceeds budget %d", len(raw), budget)
	}
}
...
func TestErrorLayerB_CursorRejectsFilterCombo(t *testing.T) {
	...
	result, err := session.CallTool(context.Background(), &mcp.CallToolParams{
		Name: "check_error_handling",
		Arguments: map[string]any{
			"cursor":        "eyJzaWQiOiJhYmMiLCJvZmYiOjAsInRvb2wiOiJjaGVja19lcnJvcl9oYW5kbGluZyJ9",
			"include_tests": true,
		},
	})
	if err != nil {
...
func TestErrorLayerA_SizeBudget(t *testing.T) {
	...
	if result.IsError {
		t.Fatalf("check_error_handling Layer A returned IsError: %+v", result)
	}
	m := errorStructuredMap(t, result)
	if got := m["response_shape"]; got != "flat" {
	...
	const budget = 20480
	t.Logf("error Layer A wire size on golden fixture (include_tests=true) = %d bytes (budget %d)", len(raw), budget)
	if len(raw) >= budget {
		t.Fatalf("error Layer A wire size %d bytes exceeds budget %d", len(raw), budget)
	}
}
...
func TestErrorToolDescription_SummaryFirstBias(t *testing.T) {
	...
	for _, n := range needles {
		if !strings.Contains(desc, n) {
			t.Errorf("check_error_handling description missing substring %q; got: %s", n, desc)
		}
	}
}
...
func TestErrorLayerB_MaxResultSizeChars(t *testing.T) {
	...
	for _, tool := range tools.Tools {
		...
		if !ok {
			t.Fatalf("check_error_handling Meta missing anthropic/maxResultSizeChars; got %+v", tool.Meta)
		}
		got := int64(0)
		switch x := v.(type) {
		case int:
