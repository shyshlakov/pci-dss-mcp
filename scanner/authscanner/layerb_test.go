...
func newAuthSessionForLayerB(t *testing.T) *mcp.ClientSession {
	t.Helper()
	server := mcp.NewServer(&mcp.Implementation{Name: "auth-layerb", Version: "v0.0.1"}, nil)
	...
	client := mcp.NewClient(&mcp.Implementation{Name: "auth-layerb-test-client", Version: "v0.0.1"}, nil)
...
func copyFixtureTreeForAuth(t *testing.T, srcRoot string) string {
	t.Helper()
...
func callAuthDefault(t *testing.T, path string) *mcp.CallToolResult {
	t.Helper()
	...
	if err != nil {
		t.Fatalf("CallTool check_auth_strength: %v", err)
	}
	if result.IsError {
		t.Fatalf("check_auth_strength IsError: %+v", result)
	}
	return result
}

func authStructuredMap(t *testing.T, result *mcp.CallToolResult) map[string]any {
	t.Helper()
...
func TestAuthLayerB_TopNPerSeverity_Is3(t *testing.T) {
	...
	if lowArr, _ := top["low"].([]any); len(lowArr) != 0 {
		t.Errorf("top_findings[\"low\"] expected empty (AUTH LOW bucket is 0 on fixture); got %d", len(lowArr))
	}
}
...
func TestAuthLayerB_SizeBudget20KB(t *testing.T) {
	...
	const budget = 20480
	t.Logf("auth Layer B wire size on golden fixture = %d bytes (budget %d)", len(raw), budget)
	if len(raw) >= budget {
		t.Fatalf("auth Layer B wire size %d bytes exceeds budget %d", len(raw), budget)
	}
}
...
func TestAuthLayerB_CursorRejectsFilterCombo(t *testing.T) {
	...
	result, err := session.CallTool(context.Background(), &mcp.CallToolParams{
		Name: "check_auth_strength",
		Arguments: map[string]any{
			"cursor":        "eyJzaWQiOiJhYmMiLCJvZmYiOjAsInRvb2wiOiJjaGVja19hdXRoX3N0cmVuZ3RoIn0",
			"include_tests": true,
		},
	})
	if err != nil {
...
func TestAuthLayerB_FilterSet_HasCursor(t *testing.T) {
	...
	if totalFindings > 30 && cur == "" {
		t.Errorf("expected non-empty next_cursor when total_findings=%d > 30 with include_tests=true; got empty cursor (findings page len=%d)", totalFindings, len(findingsArr))
	}
	if got := m["response_shape"]; got != "flat" {
...
func TestAuthToolDescription_SummaryFirstBias(t *testing.T) {
	...
	for _, n := range needles {
		if !strings.Contains(desc, n) {
			t.Errorf("check_auth_strength description missing substring %q; got: %s", n, desc)
		}
	}
}
...
func TestAuthLayerA_SizeBudget(t *testing.T) {
	...
	if result.IsError {
		t.Fatalf("check_auth_strength Layer A returned IsError: %+v", result)
	}
	m := authStructuredMap(t, result)
	if got := m["response_shape"]; got != "flat" {
	...
	const budget = 20480
	t.Logf("auth Layer A wire size on golden fixture (include_tests=true) = %d bytes (budget %d)", len(raw), budget)
	if len(raw) >= budget {
		t.Fatalf("auth Layer A wire size %d bytes exceeds budget %d", len(raw), budget)
	}
}
...
func TestAuthLayerB_MaxResultSizeChars(t *testing.T) {
	...
	for _, tool := range tools.Tools {
		...
		if !ok {
			t.Fatalf("check_auth_strength Meta missing anthropic/maxResultSizeChars; got %+v", tool.Meta)
		}
		got := int64(0)
		switch x := v.(type) {
		case int:
