...
const toolNameAudit = "audit_log_coverage"
...
type CheckAuditLogInput struct {
	...
	Cursor           string   `json:"cursor,omitempty" jsonschema:"Opaque cursor token from a prior audit_log_coverage response. When set resumes pagination from the stored session cache (10-minute TTL). Leave empty for a fresh scan."`
...
type auditCacher struct{}

func (auditCacher) Put(sid string, findings []scanner.Finding, meta hybridcache.ScanMeta) {
	cp := make([]scanner.Finding, len(findings))
	copy(cp, findings)
	hybridcache.Put(sid, &hybridcache.Entry{Findings: cp, Meta: meta})
}

func (auditCacher) Get(sid string) ([]scanner.Finding, hybridcache.ScanMeta, bool) {
...
func (auditCacher) PutWithHistogram(sid string, findings []scanner.Finding, meta hybridcache.ScanMeta, hist *hybridcache.Histogram) {
	cp := make([]scanner.Finding, len(findings))
	copy(cp, findings)
	hybridcache.PutWithHistogram(sid, cp, meta, hist)
}

func (auditCacher) GetWithHistogram(sid string) ([]scanner.Finding, hybridcache.ScanMeta, *hybridcache.Histogram, bool) {
	return hybridcache.GetWithHistogram(sid)
}

func (auditCacher) Histogram(findings []scanner.Finding) *hybridcache.Histogram {
...
func RegisterTools(server *mcp.Server) {
	s := New()
	schema, err := buildAuditOutputSchemaUnion()
	if err != nil {
		slog.Warn("buildAuditOutputSchemaUnion failed", "err", err)
	}

	tool := &mcp.Tool{
		Name: toolNameAudit,
		Description: "Scan Go source files for payment handlers missing structured audit logging (PCI DSS 10.2.1). Detects: missing logging, unstructured-only logging (fmt/log), and reports handlers with structured logging. Framework-aware: supports net/http, gin, echo handler signatures. " +
	...
	mcp.AddTool(server, tool, func(ctx context.Context, req *mcp.CallToolRequest, input CheckAuditLogInput) (*mcp.CallToolResult, any, error) {
		if input.Limit == -1 {
			return auditErrorResult("LIMIT_MINUS_ONE_REMOVED: limit=-1 is no longer accepted. For interactive use, call with default params (summary-first with next_cursor) or apply min_severity/rule_filter for a paged flat response; follow the cursor for subsequent pages."), nil, nil
		}
		...
		if input.Cursor != "" && (scopeFilterSet || qualityFilterSet) {
			return auditErrorResult("audit_log_coverage cursor_malformed: cursor + filter/scope params is not supported; re-run without cursor to apply new filters"), nil, nil
		}
		...
		buildSummary := func(findings []scanner.Finding, meta hybridcache.ScanMeta, sid, nextCursor string) *AuditSummaryResponse {
			return buildAuditSummaryInternal(findings, meta, sid, nextCursor)
		}
		...
		in := hybrid.Input{
			...
			ToolName:      toolNameAudit,
			FlatPageSize:  30,
		}

		res, sErr := hybrid.SelectAndExecute[scanner.Finding, AuditSummaryResponse, scanner.ScannerToolOutput](
			ctx, in, scan, filterFunc, buildSummary, buildFlat, auditCacher{},
		)
		if sErr != nil {
			return auditErrorResult(fmt.Sprintf("audit_log_coverage error: %s", sErr.Error())), nil, nil
		}
		if res.Err != nil {
			cerr := &AuditCursorError{
				ResponseShape: "error",
		...
		if res.Summary != nil {
			return &mcp.CallToolResult{
				Content: []mcp.Content{&mcp.TextContent{Text: fmt.Sprintf("audit_log_coverage: %d findings (summary with top %d per severity)", res.Summary.Pagination.TotalFindings, TopNPerSeverityAudit)}},
			}, res.Summary, nil
		}
		return &mcp.CallToolResult{
			Content: []mcp.Content{&mcp.TextContent{Text: fmt.Sprintf("audit_log_coverage: %d findings returned (flat page)", len(res.Flat.Findings))}},
		}, res.Flat, nil
	})
}
...
func auditErrorResult(msg string) *mcp.CallToolResult {
	return &mcp.CallToolResult{
