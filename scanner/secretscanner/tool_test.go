package secretscanner_test

import (
	"context"
	"os"
	"path/filepath"
	"testing"

	"github.com/modelcontextprotocol/go-sdk/mcp"

	"github.com/shyshlakov/pci-dss-mcp/scanner"
	"github.com/shyshlakov/pci-dss-mcp/scanner/secretscanner"
)

// setupTestServer creates a server with check_secrets_in_configs registered,
// connects an in-memory client, and returns the client session.
func setupTestServer(t *testing.T) *mcp.ClientSession {
	t.Helper()

	server := mcp.NewServer(&mcp.Implementation{Name: "pci-dss-mcp-test", Version: "v0.0.1"}, nil)
	secretscanner.RegisterTools(server)

	clientTransport, serverTransport := mcp.NewInMemoryTransports()

	// Start server in background.
	ctx, cancel := context.WithCancel(context.Background())
	t.Cleanup(cancel)
	go func() {
		if err := server.Run(ctx, serverTransport); err != nil {
			return
		}
	}()

	client := mcp.NewClient(&mcp.Implementation{Name: "test-client", Version: "v0.0.1"}, nil)
	session, err := client.Connect(context.Background(), clientTransport, nil)
	if err != nil {
		t.Fatalf("client.Connect() failed: %v", err)
	}
	t.Cleanup(func() { session.Close() })

	return session
}

// extractToolText extracts the text from the first TextContent in a CallToolResult.
func extractToolText(t *testing.T, result *mcp.CallToolResult) string {
	t.Helper()
	if len(result.Content) == 0 {
		t.Fatal("CallToolResult has no content")
	}
	tc, ok := result.Content[0].(*mcp.TextContent)
	if !ok {
		t.Fatalf("Expected TextContent, got %T", result.Content[0])
	}
	return tc.Text
}

func TestToolRegistered(t *testing.T) {
	session := setupTestServer(t)

	result, err := session.ListTools(context.Background(), &mcp.ListToolsParams{})
	if err != nil {
		t.Fatalf("ListTools failed: %v", err)
	}

	found := false
	for _, tool := range result.Tools {
		if tool.Name == "check_secrets_in_configs" {
			found = true
			if tool.Description == "" {
				t.Error("check_secrets_in_configs tool should have a non-empty description")
			}
			break
		}
	}
	if !found {
		t.Error("check_secrets_in_configs tool should be registered in the server")
	}
}

func TestToolValidPath(t *testing.T) {
	session := setupTestServer(t)

	// Copy fixtures to a temp dir (walker excludes "testdata" directories).
	fixtureDir := filepath.Join("..", "..", "testdata")
	files := []string{"secrets.env", "secrets.yaml", "secrets.json", "secrets.toml"}

	tmpDir := t.TempDir()
	for _, f := range files {
		src := filepath.Join(fixtureDir, f)
		data, err := os.ReadFile(src)
		if err != nil {
			t.Skipf("fixture %s not found, skipping: %v", f, err)
		}
		if err := os.WriteFile(filepath.Join(tmpDir, f), data, 0644); err != nil {
			t.Fatalf("WriteFile %s: %v", f, err)
		}
	}

	result, err := session.CallTool(context.Background(), &mcp.CallToolParams{
		Name: "check_secrets_in_configs",
		Arguments: map[string]any{
			"path":          tmpDir,
			"include_tests": true,
		},
	})
	if err != nil {
		t.Fatalf("CallTool failed: %v", err)
	}
	if result.IsError {
		t.Fatalf("Expected success, got IsError=true: %s", extractToolText(t, result))
	}

	out := parseOut(t, result)
	if !out.HasSeverity(scanner.SeverityCritical) {
		t.Errorf("Expected CRITICAL findings, got stats: %+v", out.SeverityStats)
	}
}

func TestToolInvalidPath(t *testing.T) {
	session := setupTestServer(t)

	result, err := session.CallTool(context.Background(), &mcp.CallToolParams{
		Name: "check_secrets_in_configs",
		Arguments: map[string]any{
			"path": "/nonexistent/path/that/does/not/exist",
		},
	})
	if err != nil {
		t.Fatalf("CallTool failed: %v", err)
	}
	if !result.IsError {
		t.Fatal("Expected IsError=true for non-existent path")
	}
}

func TestToolExcludePatterns(t *testing.T) {
	session := setupTestServer(t)

	// Create a temp dir with a secrets file.
	tmpDir := t.TempDir()
	if err := os.WriteFile(filepath.Join(tmpDir, "test.env"), []byte("API_KEY=sk_live_test1234567890abcdefghijk\n"), 0644); err != nil {
		t.Fatalf("WriteFile: %v", err)
	}

	// Exclude all.env files -- should find nothing.
	result, err := session.CallTool(context.Background(), &mcp.CallToolParams{
		Name: "check_secrets_in_configs",
		Arguments: map[string]any{
			"path":             tmpDir,
			"exclude_patterns": []any{"*.env"},
		},
	})
	if err != nil {
		t.Fatalf("CallTool failed: %v", err)
	}
	if result.IsError {
		t.Fatalf("Expected success, got IsError=true: %s", extractToolText(t, result))
	}

	out := parseOut(t, result)
	if len(out.Findings) != 0 {
		t.Errorf("Expected 0 findings when all config files excluded, got %d: %+v", len(out.Findings), out.Findings)
	}
}
