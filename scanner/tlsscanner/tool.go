...
const toolNameTLS = "check_tls_config"
...
type CheckTLSInput struct {
	...
	Cursor           string   `json:"cursor,omitempty" jsonschema:"Opaque cursor token from a prior check_tls_config response. When set resumes pagination from the stored session cache (10-minute TTL). Leave empty for a fresh scan."`
...
type tlsCacher struct{}

func (tlsCacher) Put(sid string, findings []scanner.Finding, meta hybridcache.ScanMeta) {
	cp := make([]scanner.Finding, len(findings))
	copy(cp, findings)
	hybridcache.Put(sid, &hybridcache.Entry{Findings: cp, Meta: meta})
}

func (tlsCacher) Get(sid string) ([]scanner.Finding, hybridcache.ScanMeta, bool) {
...
func (tlsCacher) PutWithHistogram(sid string, findings []scanner.Finding, meta hybridcache.ScanMeta, hist *hybridcache.Histogram) {
	cp := make([]scanner.Finding, len(findings))
	copy(cp, findings)
	hybridcache.PutWithHistogram(sid, cp, meta, hist)
}

func (tlsCacher) GetWithHistogram(sid string) ([]scanner.Finding, hybridcache.ScanMeta, *hybridcache.Histogram, bool) {
	return hybridcache.GetWithHistogram(sid)
}

func (tlsCacher) Histogram(findings []scanner.Finding) *hybridcache.Histogram {
...
func RegisterTools(server *mcp.Server) {
	s := New()
	schema, err := buildTLSOutputSchemaUnion()
	if err != nil {
		slog.Warn("buildTLSOutputSchemaUnion failed", "err", err)
	}

	tool := &mcp.Tool{
		Name: toolNameTLS,
		Description: "Scan Go source files for TLS configuration violations: InsecureSkipVerify, weak MinVersion (below TLS 1.2), missing MinVersion, and prohibited cipher suites (RC4, 3DES, NULL). " +
			...
			"Maps findings to PCI DSS 4.2.1.",
		Meta:         mcp.Meta{"anthropic/maxResultSizeChars": 20000},
		OutputSchema: schema,
	}

	mcp.AddTool(server, tool, func(ctx context.Context, req *mcp.CallToolRequest, input CheckTLSInput) (*mcp.CallToolResult, any, error) {
		if input.Limit == -1 {
			return tlsErrorResult("LIMIT_MINUS_ONE_REMOVED: limit=-1 is no longer accepted. For interactive use, call with default params (summary-first with next_cursor) or apply min_severity/rule_filter for a paged flat response; follow the cursor for subsequent pages."), nil, nil
		}
		...
		if input.Cursor != "" && (scopeFilterSet || qualityFilterSet) {
			return tlsErrorResult("check_tls_config cursor_malformed: cursor + filter/scope params is not supported; re-run without cursor to apply new filters"), nil, nil
		}
		...
		buildSummary := func(findings []scanner.Finding, meta hybridcache.ScanMeta, sid, nextCursor string) *TLSSummaryResponse {
			return buildTLSSummaryInternal(findings, meta, sid, nextCursor)
		}
		...
		in := hybrid.Input{
			...
			ToolName:      toolNameTLS,
			FlatPageSize:  30,
		}

		res, sErr := hybrid.SelectAndExecute[scanner.Finding, TLSSummaryResponse, scanner.ScannerToolOutput](
			ctx, in, scan, filterFunc, buildSummary, buildFlat, tlsCacher{},
		)
		if sErr != nil {
			return tlsErrorResult(fmt.Sprintf("check_tls_config error: %s", sErr.Error())), nil, nil
		}
		if res.Err != nil {
			cerr := &TLSCursorError{
				ResponseShape: "error",
		...
		if res.Summary != nil {
			return &mcp.CallToolResult{
				Content: []mcp.Content{&mcp.TextContent{Text: fmt.Sprintf("check_tls_config: %d findings (summary with top %d per severity)", res.Summary.Pagination.TotalFindings, TopNPerSeverityTLS)}},
			}, res.Summary, nil
		}
		return &mcp.CallToolResult{
			Content: []mcp.Content{&mcp.TextContent{Text: fmt.Sprintf("check_tls_config: %d findings returned (flat page)", len(res.Flat.Findings))}},
		}, res.Flat, nil
	})
}
...
func tlsErrorResult(msg string) *mcp.CallToolResult {
	return &mcp.CallToolResult{
