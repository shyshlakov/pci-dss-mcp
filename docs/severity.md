...
### HTTP Input Taint Scanner (httpinputscanner)

| Rule ID | Severity | Requirement | Description |
|---------|----------|-------------|-------------|
| HTTP-INPUT-ERROR | MEDIUM (HIGH on Stringer-receiver auth-secret type name) | 6.2.4 (dynamic -- see docs/requirement-mapping.md) | Framework input baked into `fmt.Errorf`, written to `http.ResponseWriter`, surfaced through a centralized abort helper, or passed to `(*gin.Context).AbortWithError`. Severity = three-class dispatch on source identifier name (see docs/http_input_taint.md "Severity-aware emission"). PAN/CHD or auth-secret keyword -> HIGH; default -> MEDIUM |
| HTTP-INPUT-LOG | CRITICAL / HIGH / MEDIUM / suppressed | 10.2.1, 3.3.1/3.5.1, 8.6.2, or 3.4.1/8.6.2 (dynamic -- see docs/requirement-mapping.md) | Framework input flowing into a log sink without a sanitizer barrier. Severity = three-class dispatch on source identifier OR sink-key literal (see docs/http_input_taint.md "Severity-aware emission"). PAN/CHD or auth-secret keyword -> HIGH; generic-ID keyword (request_id/trace_id/widget_id/tenant_id/merchant_id/correlation_id/span_id) -> suppressed; default -> MEDIUM. CRITICAL fires when the sink directly receives `validator.FieldError.Value()` AND the bound struct has a PAN-tagged field (PAN-validation profile, related [3.4.1, 8.6.2]) |
| HTTP-INPUT-PANIC | MEDIUM | 10.2.1 or 6.2.4 (dynamic -- see docs/requirement-mapping.md) | Framework input reaching `panic(...)` or a `defer recover()` re-log path; gin.CustomRecoveryWithWriter / CustomRecovery / RecoveryWithWriter `recovered any` callback parameter recognized as auxiliary source. Bare panic dedup: a file installing a gin recovery callback sink suppresses bare panic emissions in the same file |
