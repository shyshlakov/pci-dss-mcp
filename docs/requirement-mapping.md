...
| HTTP-INPUT-ERROR | 6.2.4 |  | httpinputscanner | full | dynamic emit; raw HTTP framework input baked into fmt.Errorf, written to ResponseWriter, surfaced via centralized abort helper, or passed to (*gin.Context).AbortWithError; 6.2.4 improper-error-handling violation. Stringer-receiver auth-secret type name promotes HIGH + [8.6.2] |
| HTTP-INPUT-LOG | 10.2.1 |  | httpinputscanner | full | dynamic emit; HTTP framework input flowing into a log sink without sanitizer barrier. Severity / related-reqs by class: PAN/CHD class -> HIGH + [3.3.1, 3.5.1]; auth-secret class -> HIGH + [8.6.2]; PAN-validation profile (validator.FieldError.Value() direct arg with bound struct PAN tag) -> CRITICAL + [3.4.1, 8.6.2]; generic-ID class (request_id / trace_id / widget_id / tenant_id / merchant_id / correlation_id / span_id) -> suppressed; default body / header -> MEDIUM. apikey reclassified from PAN-keyword (related [3.3.1, 3.5.1]) to auth-secret (related [8.6.2]) BREAKING change in v0.7.1 |
| HTTP-INPUT-PANIC | 10.2.1 |  | httpinputscanner | full | dynamic emit; HTTP framework input reaching panic(...), defer recover() re-log, or gin.CustomRecoveryWithWriter / CustomRecovery / RecoveryWithWriter callback parameter; recovery middleware will log the panic value, violating 10.2.1 audit content. Bare panic dedup: a file installing a gin recovery callback sink suppresses bare panic emissions in the same file |
